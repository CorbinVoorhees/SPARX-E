...
public:
        ...
        : Node("lidar_obstacle_processor")
    {
        ...
        publication_ = this->create_publisher<lidar_processor::msg::Env>(
            "/obstacles", 10
        );

        // Timer to process the accumulated cloud and write CSV every 0.5 seconds (500ms)
        processing_timer_ = this->create_wall_timer(
            2000ms, std::bind(&LidarObstacleProcessor::timerCallback, this)
        );

        this->estimation_subscription = this->create_subscription<nav_msgs::msg::Odometry>(
            "/trans_est", 10,
            std::bind(
                &LidarObstacleProcessor::estimation_callback,
                this,
                std::placeholders::_1
            )
        );
...
private:
    ...
    pcl::PointCloud<pcl::PointXYZ>::Ptr accumulated_cloud_; // New buffer for accumulating points
    ...
    rclcpp::Subscription<nav_msgs::msg::Odometry>::SharedPtr estimation_subscription;
    ...
    void pointCloudCallback(const sensor_msgs::msg::PointCloud2::SharedPtr msg)
    {
        ...
        // Lock mutex and append the newly transformed points to the accumulator
        {
            ...
            *accumulated_cloud_ += temp_cloud; // PCL supports direct point cloud concatenation
    ...
    void timerCallback()
    {
        ...
        // 3. Write results to CSV safely
        {
            ...
            for (const auto& i : obstacles_) {
                ...
                storage << "," << orientation[0] << "," << orientation[1] << "," << orientation[2];
                storage << "," << velocity[0] << "," << velocity[1] << "," << velocity[2];
    ...
    void filterPoints()
    {
        ...
        for (const auto& p : cloud_body_->points) {
            ...
            if (p.z > projector_height) continue;
