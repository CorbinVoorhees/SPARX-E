...
// Merged MEKF + TRANSEKF into a single filter.
//
// Nominal state:
...
class CHUUDEKF : public ckf::CommonKF {
private:
  ...
  void predict(double dt) override {
    ...
    // ---- TRANSEKF predict (position / velocity / mu), verbatim ----
    ...
    // dfdx indices shifted +3 for the attitude prefix (was 0:8 in TRANSEKF's
    // own state; here position=3:6, velocity=6:9, mu=9:11). attitude block
    // (0:3) left as identity — already propagated above, own P block.
    ...
    // d(velocity)/d(mu_c) uses (wr+wl); d/d(mu_d) uses (wr-wl)
    dfdx.block<3, 1>(6, 9) = R_I_B.col(0) * GEOM.rw / 2 * (wr + wl);
    dfdx.block<3, 1>(6, 10) = R_I_B.col(0) * GEOM.rw / 2 * (wr - wl);
    // d(position)/d(mu) = d(velocity)/d(mu) * dt. WITHOUT this term, mu only
    // couples to position through the two-hop velocity path, so a UWB position
    // residual barely reaches mu (mu stays frozen during steady cruise, where
    // accel gives it no info either). This direct term lets UWB range — which
    // measures true distance — correct mu_c: if the rover goes further than the
    // wheels predict, mu_c scales up. This is the forward-slip observer.
    dfdx.block<3, 1>(3, 9) = R_I_B.col(0) * GEOM.rw / 2 * (wr + wl) * dt;
    dfdx.block<3, 1>(3, 10) = R_I_B.col(0) * GEOM.rw / 2 * (wr - wl) * dt;
    dfdx(9, 9) = 1.0;
    dfdx(10, 10) = 1.0;

    Eigen::MatrixXd Qd = Eigen::MatrixXd::Zero(11, 11);
    // Wheel scale varies with slip and terrain. Keep enough process noise on
    // mu_c/mu_d for UWB and acceleration corrections to move the estimate
    // after startup.
    Qd.diagonal().segment<8>(3) << 1e-2, 1e-4, 1e-6, 1e-3, 1e-3, 1e-3, 1e-5,
        1e-5;
...
public:
  ...
  CHUUDEKF(nav::NavigationSensors &sensors, SteadyClock::time_point t0,
           Eigen::Vector3d pos0)
    ...
    // The initial position is measured and the rover is known to be at rest,
    // so both begin tightly constrained. Wheel scale is less certain: equal
    // 1e-4 variances on mu_c/mu_d give each derived wheel scale an initial
    // standard deviation of about 0.014.
    this->P.diagonal().segment<3>(3) << 1e-4, 1e-4, 1e-4; // p: sigma = 0.01 m
    this->P.diagonal().segment<3>(6) << 1e-6, 1e-6, 1e-6; // v: sigma = 0.001 m/s
    this->P.diagonal().segment<2>(9) << 1e-4, 1e-4;       // wheel scale

    // ================= MEKF accel (attitude) =================
    sensors.bind<nav::AccelerometerSensor>(
    ...
    // ================= control =================
    sensors.bind<nav::ControlSensor>(
        [this](const V4 &data, const Eigen::MatrixXd &,
          ...
          const double dwr_at_t = data(2);
          const double dwl_at_t = data(3);
          const V3 z = accel_snapshot.z;
          const V3 gyro_at_t = gyro_snapshot.z.head<3>();
          const V3 gyro_bias_at_t = gyro_bias();
          const V3 grav_at_t = grav_ref;
          const Eigen::Matrix3d R_B_I_at_t =
              rel_orientation().normalized().toRotationMatrix().transpose();
          const Eigen::MatrixXd R = 1e-2 * Eigen::Matrix3d::Identity();

          queue_correction(
              t, R,
              [this, z, dwr_at_t, dwl_at_t, gyro_at_t, gyro_bias_at_t,
               grav_at_t, R_B_I_at_t](Eigen::MatrixXd &H, Eigen::VectorXd &r,
                                      Eigen::VectorXd &z_out) {
                ...
                H = Eigen::MatrixXd::Zero(3, P.cols());
                H.block<3, 3>(0, 6) = skew(omega) * R_B_I_at_t;
                H.col(9) = -V3::UnitX() * GEOM.rw / 2 * (dwr_at_t + dwl_at_t) /
                           CTRL_PERIOD_S;
                H.col(10) = -V3::UnitX() * GEOM.rw / 2 * (dwr_at_t - dwl_at_t) /
                            CTRL_PERIOD_S;
          ...
          queue_correction(
              ...
              [this, wr_at_t, wl_at_t, gyro_at_t, gyro_bias_at_t,
               R_I_B_at_t](Eigen::MatrixXd &H, Eigen::VectorXd &r,
                ...
                // CommonKF expects r = z - h(x): gyro yaw rate is the
                // measurement and wheel-derived yaw rate is the prediction.
                // Reversing this residual turns the update into positive
                // feedback on mu_d.
                r = Eigen::VectorXd::Constant(1, omega_z - psi_dot_wheel);
    ...
    // ================= gyro =================
    sensors.bind<nav::GyroscopeSensor>(
